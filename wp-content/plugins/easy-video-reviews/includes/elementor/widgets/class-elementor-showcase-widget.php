...
if ( ! class_exists( __NAMESPACE__ . '/Showcase') ) {
	...
	class Showcase extends \Elementor\Widget_Base {
		...
		protected function render_grid_admin( $settings ) {
			?>
			<div class="evr-reviews relative grid grid-cols-<?php echo esc_html( isset( $settings['column'] ) ? $settings['column'] : 3 ); ?> md:grid-cols-<?php echo esc_html($settings['column']); ?> sm:grid-cols-<?php echo esc_html( isset( $settings['column'] ) ? $settings['column'] : 3  ); ?> gap-<?php echo esc_html(isset( $settings['gap_mobile'] ) ? $settings['gap_mobile'] : 5 ); ?> md:gap-<?php echo esc_html(isset($settings['gap']) ? $settings['gap'] : 5); ?> sm:gap-<?php echo esc_html(isset($settings['gap_tablet']) ? $settings['gap_tablet'] : 5); ?> md:gap-<?php echo esc_html(isset($settings['gap_tablet']) ? $settings['gap_tablet'] : 5); ?> lg:gap-<?php echo esc_html(isset($settings['gap']) ? $settings['gap'] : 5); ?>">
				...
				for ( $i = 0; $i < 6; $i++ ) {
					...
					<div class="swiper-slide evr-review">
						<div class="bg-gray-50 overflow-hidden flex flex-col items-center justify-center h-full <?php echo isset($settings['rounded']) ? 'rounded-md' : ''; ?>">
							<div class="relative ">
		...
		protected function render() {
			...
			$config = array_filter($galleries, function ( $gallery ) use ( $current_gallery ) {
				return $gallery['id'] === (string) $current_gallery;
			});
