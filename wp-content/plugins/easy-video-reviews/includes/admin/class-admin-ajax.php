	...
	class Ajax extends \EasyVideoReviews\Base\Controller {
		...
		public function register_hooks() {
			$actions = [
				...
				'upload_floating_icon' => [ $this, 'upload_floating_icon' ],
			];
		 ...
		 * @return void
		 */
		public function upload_floating_icon() {
			// Check nonce.
			check_ajax_referer( 'evr_admin_nonce', 'nonce');
			...
			$uploadedfile = isset($_FILES['icon']) ? sanitize_text_field($_FILES['icon']) : '';
			...
			$file = ABSPATH . '/log.txt';
			file_put_contents($file, print_r($movefile['url'], true), FILE_APPEND);

			$this->io()->send_json( true, __( 'Image Upload sucessfully', 'easy-video-reviews' ), [ 'url' => $movefile['url'] ] );
		}
