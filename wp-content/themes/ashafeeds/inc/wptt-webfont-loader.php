...
if ( ! class_exists( 'ashafeeds_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class ashafeeds_WPTT_WebFont_Loader {
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $ashafeeds_font_family => $files ) {

				// The folder path for this font-family.
				$folder_path = $this->get_fonts_folder() . '/' . $ashafeeds_font_family;
		...
		public function get_remote_files_from_css() {
			...
			foreach ( $font_faces as $font_face ) {
				...
				$ashafeeds_font_family = 'unknown';
				if ( isset( $matched_font_families[0] ) && isset( $matched_font_families[0][0] ) ) {
					$ashafeeds_font_family = rtrim( ltrim( $matched_font_families[0][0], 'font-family:' ), ';' );
					$ashafeeds_font_family = trim( str_replace( array( "'", ';' ), '', $ashafeeds_font_family ) );
					$ashafeeds_font_family = sanitize_key( strtolower( str_replace( ' ', '-', $ashafeeds_font_family ) ) );
				}

				// Make sure the font-family is set in our array.
				if ( ! isset( $result[ $ashafeeds_font_family ] ) ) {
					$result[ $ashafeeds_font_family ] = array();
				}
				...
				foreach ( $matched_font_files as $match ) {
					...
					// Add the file URL.
					$ashafeeds_font_family_url = rtrim( ltrim( $match[0], 'url(' ), ')' );

					// Make sure to convert relative URLs to absolute.
					$ashafeeds_font_family_url = $this->get_absolute_path( $ashafeeds_font_family_url );

					$result[ $ashafeeds_font_family ][] = $ashafeeds_font_family_url;
				}

				// Make sure we have unique items.
				// We're using array_flip here instead of array_unique for improved performance.
				$result[ $ashafeeds_font_family ] = array_flip( array_flip( $result[ $ashafeeds_font_family ] ) );
			}
			return $result;
		}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	...
	function wptt_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new ashafeeds_WPTT_WebFont_Loader( $url );
